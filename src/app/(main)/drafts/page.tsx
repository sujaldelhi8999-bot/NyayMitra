...
export default function DraftsPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-5 py-8 text-white sm:px-8">
        <header className="rounded-lg bg-gradient-to-br from-teal-600 to-slate-900 p-8 shadow-2xl">
          <p className="inline-flex rounded-lg bg-white/15 px-4 py-2 text-sm font-black">{t("draftsPageTitle")}</p>
          <h1 className="mt-5 text-4xl font-black tracking-tight sm:text-6xl">{t("appName")} {t("draftsPageTitle")}</h1>
          <p className="mt-3 max-w-3xl text-lg text-slate-100">{t("draftsManage")}</p>
          <p className="mt-3 rounded-lg bg-slate-950/70 p-4 text-sm font-semibold text-slate-200">NyayMitra is a legal self-help tool, not a lawyer. Verify with legal aid/lawyer.</p>
        </header>

        {cases.length === 0 ? (
          <section className="mt-8 rounded-lg bg-white p-10 text-center text-slate-950 shadow-2xl">
            <h2 className="text-3xl font-black">{t("noDrafts")}</h2>
            <p className="mt-3 text-slate-600">{t("noDraftsPrompt")}</p>
            <Link href="/intake" className="mt-6 inline-flex rounded-lg bg-teal-600 px-6 py-3.5 font-black text-white min-h-[48px]">{t("startNewCase")}</Link>
          </section>
        ) : (
          <>
            <section className="mt-8 grid gap-4 grid-cols-2 sm:grid-cols-4">
              <Stat title={t("statTotalDrafts")} value={String(cases.length)} />
              <Stat title={t("statDraftReady")} value={String(draftReady)} />
              <Stat title={t("statDraftInProgress")} value={String(inProgress)} />
            ...
            <section className="mt-8 grid gap-5 lg:grid-cols-2">
              {filteredCases.map((caseData) => {
                ...
                const draftPreview = (caseData.complaintDraft || caseData.story || "").slice(0, 120);
                  ...
                  <article key={caseData.caseId} className="rounded-lg bg-white p-6 text-slate-950 shadow-2xl">
                    ...
                    {draftPreview && (
                      <div className="mt-4 rounded-lg bg-slate-50 p-4">
                        <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">{t("draftPreview")}</p>
                        <p className="mt-1 text-sm font-semibold text-slate-700">{draftPreview}{(caseData.complaintDraft || caseData.story || "").length > 120 ? "..." : ""}</p>
                      </div>
                    )}
