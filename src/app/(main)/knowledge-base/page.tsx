    ...
    <div className="min-h-screen bg-slate-950 px-5 py-8 text-white sm:px-8">
      <div className="mx-auto max-w-7xl">
        <header className="rounded-[2rem] bg-gradient-to-br from-teal-500 to-slate-900 p-8 shadow-2xl">
          <p className="inline-flex rounded-full bg-white/15 px-4 py-2 text-sm font-black">{t("kbHeader")}</p>
          ...
          <p className="mt-4 rounded-2xl bg-slate-950/70 p-4 text-sm font-semibold">{t("kbDisclaimer")}</p>
        </header>

        <section className="mt-8 rounded-3xl bg-white p-5 text-slate-950 shadow-2xl">
          <div className="flex flex-wrap gap-3">
            <button type="button" onClick={() => setTab("legal")} className={`rounded-full px-5 py-3 font-black ${tab === "legal" ? "bg-slate-950 text-white" : "bg-slate-100 text-slate-700"}`}>{t("tabLegalKnowledge")}</button>
            <button type="button" onClick={() => setTab("portals")} className={`rounded-full px-5 py-3 font-black ${tab === "portals" ? "bg-teal-600 text-white" : "bg-slate-100 text-slate-700"}`}>{t("tabOfficialPortals")}</button>
          </div>
          {tab === "legal" && <><label className="mt-5 block font-black text-teal-700">{t("filterByCaseType")}</label><select value={filter} onChange={(event) => setFilter(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 p-3 font-bold outline-none focus:border-teal-500 md:max-w-md">
            {caseTypes.map((caseType) => <option key={caseType}>{caseType}</option>)}
          </select></>}
          {tab === "portals" && <><label className="mt-5 block font-black text-teal-700">{t("filterByCaseType")}</label><select value={portalFilter} onChange={(event) => setPortalFilter(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 p-3 font-bold outline-none focus:border-teal-500 md:max-w-md">
        ...
        {tab === "legal" ? <section className="mt-8 grid gap-5 md:grid-cols-2">
          ...
          {filteredPortals.map((portal) => (
            <article key={portal.id} className="rounded-[2rem] bg-white p-6 text-slate-950 shadow-2xl">
              <div className="flex flex-wrap gap-2"><span className="rounded-full bg-teal-100 px-3 py-1 text-xs font-black text-teal-800">{t("badgeOfficial")}</span>{portal.emergencyOnly && <span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-black text-orange-800">{t("badgeEmergency")}</span>}{portal.stateSpecific && <span className="rounded-full bg-yellow-100 px-3 py-1 text-xs font-black text-yellow-800">{t("badgeStateSpecific")}</span>}<span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-700">{portal.category}</span></div>
